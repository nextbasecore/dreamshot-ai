...
const ClearAccountDialog = () => {
  ...
  const isOpen = dialog.includes("clearData")
  ...
  const clearAccount = async () => {
    try {
      ...
      handleDialogType("clearData", "remove")
    } catch {
      customToast.error("Failed to clear account")
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <DialogTitle className="sr-only">Clear Account</DialogTitle>
        <form
        ...
        >
          <div className='flex flex-col items-center text-center space-y-3 sm:space-y-4 p-6 sm:p-8 bg-white rounded-lg'>
            <div className='w-12 h-12 sm:w-16 sm:h-16 bg-gray-100 border-2 border-gray-200 rounded-full flex items-center justify-center'>
              <ClearAllDataIcon className="w-5 h-5 sm:w-6 sm:h-6 text-headerBG" stroke="currentColor" />
            </div>
